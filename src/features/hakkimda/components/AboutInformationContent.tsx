...
export default function AboutInformativeContent({ videos, makaleler }: Props) {
        ...
        <div className="space-y-20">
          ...
          <div>
            ...
            {previewVideos.map((video) => (
              <Link key={video._id} href="/videolarim">
                <Card className="group hover:shadow-xl transition-all duration-500 border-0 shadow-md bg-white/80 backdrop-blur-sm hover:bg-white cursor-pointer overflow-hidden mb-6">
                  <CardContent className="p-6">
                    <div className="flex gap-6">
                      <div className="relative w-48 h-28 rounded-xl overflow-hidden flex-shrink-0 shadow-md">
                        <LiteYouTubeEmbed
                          id={video.youtubeId}
                          title={video.title}
                          noCookie={true}
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-gray-900 text-lg group-hover:text-red-600 line-clamp-2 mb-3 leading-snug">
                          {video.title}
                        </h4>
          ...
          <div>
            ...
            {previewYazilar.map((yazi) => (
              <Link key={yazi._id} href={`/makalelerim/${yazi.slug}`}>
                <Card className="group hover:shadow-xl transition-all duration-500 border-0 shadow-md bg-white/80 backdrop-blur-sm hover:bg-white cursor-pointer overflow-hidden mb-6">
                  <CardContent className="p-6">
                    <div className="flex gap-6">
                      <div className="relative w-48 h-28 bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl overflow-hidden flex-shrink-0 shadow-md">
                        <Image
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-gray-900 text-lg group-hover:text-emerald-600 line-clamp-2 mb-3 leading-snug">
                          {yazi.title}
                        </h4>
                        <div className="flex items-center justify-between text-sm text-gray-500">
                          <div className="flex items-center">
